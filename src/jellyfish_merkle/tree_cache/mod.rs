...
struct FrozenTreeCache<K,V> {
    /// Immutable node_cache.
    node_cache: BTreeMap<NodeKey, Node<K,V>>,
...
pub struct TreeCache<'a, R: 'a + TreeReader<K,V>, K,V> {
    ...
    frozen_cache: FrozenTreeCache<K,V>,
    ...
    K: Key, V: Value
{
    ...
    #[cfg(test)]
    pub fn get_root_node(&self) -> Result<Node<K,V>> {
    ...
    pub fn put_node(&mut self, node_key: NodeKey, new_node: Node<K,V>) -> Result<()> {
